...
exports.formatDate = (n, locale = 'en-KE') => { 
         let d = new Date(n) 
...
exports.getSizeMedia = (path) => { 
     return new Promise((resolve, reject) => { 
         if (/http/.test(path)) { 
             ...
             reject('error reading media size') 
         } 
     }) 
} 
...
exports.smsg = (conn, m, store) => { 
     ...
     if (m.key) { 
         ...
         // 🛠️ CRITICAL FIX: Safe Decode Jid
         const rawSender = m.fromMe && conn.user.id || m.participant || m.key.participant || m.chat || '';
         m.sender = conn.decodeJid ? conn.decodeJid(rawSender) : rawSender;

         if (m.isGroup) {
             const rawPart = m.key.participant;
             m.participant = conn.decodeJid ? conn.decodeJid(rawPart) : (rawPart || '');
         }
     } 
     if (m.message) { 
         ...
         if (m.quoted) { 
             ...
             // 🛠️ CRITICAL FIX: Safe Quoted Sender
             const rawQuotedSender = m.msg.contextInfo.participant;
             m.quoted.sender = conn.decodeJid ? conn.decodeJid(rawQuotedSender) : rawQuotedSender;
